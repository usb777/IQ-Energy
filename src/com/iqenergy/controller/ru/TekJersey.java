...
@Path("/ru/tek/")
public class TekJersey {
	
PageDAO pageDAO = new PageDAO();
Page tekPage =new Page();
 ...
 */
	...
	{   
	    ...
	    tekPage = pageDAO.getPageByName("tek");
		...
		session.setAttribute("menuTitle",tekPage.getPage_title());
		
		request.setAttribute("tekinfo", tekPage ); // send message to JSP    
		return new Viewable("/tek", null);
	}

	  
	 @GET
	    @Path("/{tekID}")
	    public Viewable feed_page( @Context HttpServletRequest request,   @Context HttpServletResponse response, @PathParam("tekID") int tekID) throws Exception
	    {
		 //TODO in future Remove this Costyl
		  request.setAttribute("menu_icemix", pageDAO.getAllIcemixPages() );          //TODO in future Remove this Costyl
		  request.setAttribute("menu_ecobarrier", pageDAO.getAllEcobarrierPages() );  //TODO in future Remove this Costyl
	           
		 tekPage = pageDAO.getPageById(tekID);
	      request.setAttribute("tekinfo", tekPage); // send message to JSP	      
